OLD NOTE FROM app/model/README:
--------------------------------------------------------------------------
Customizations of a model (or models) used from the standalone model/physics lib (../models)


NOTES FROM THE OLD Model/README (edited):
--------------------------------------------------------------------------
*
...
Even Math may need to be split into generic ubiquitous services from the Engine,
and optional specialized extra services needed (and provided) by a particular
model (or models — so, a math pack can be shared, too!).

But, because "math" in general is almost exclusively used only for modelling,
and partly because it's too early yet, it seems safer to start out with just
having a local Model/Math component (thus considered "specialized", despite
math being inherently universal!), instead of risking to prematurely lock in
a half-baked global math API up in the Engine. (The same is even more true
for physics, of course.)

OTOH, the imminent app-engine separation, and the need for some sort of math
in various engine component impl.s kinda urges to still move it to the engine,
after all...
