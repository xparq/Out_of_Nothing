...
if (cfg.DEBUG_show_keycode) Note("key code: " + keycode); //!! SFML3 has started making things harder every day... :-/
		...
		switch (keycode)
		{
		case Enter:     timestep_start(keystate(Shift) ?  10 :  1); break;
		case Backspace: timestep_start(keystate(Shift) ? -10 : -1); break;

		case Tab: toggle_interact_n2n(); break;

		case Insert: spawn(player_entity_ndx(),
				keystate(Shift) ? 100 : keystate(Ctrl) ? 10 : 1); break;
		case Delete: remove_random_bodies(
				keystate(Shift) ? 100 : keystate(Ctrl) ? 10 : 1); break;

//#586:		case F1:  keystate(Shift) ? quick_load_snapshot(1) : quick_save_snapshot(1); break;
		case F1: toggle_help(); break; // See also '?'!

		case F2:  keystate(Shift) ? quick_load_snapshot(2) : quick_save_snapshot(2); break;
		case F3:  keystate(Shift) ? quick_load_snapshot(3) : quick_save_snapshot(3); break;
		case F4:  keystate(Shift) ? quick_load_snapshot(4) : quick_save_snapshot(4); break;
		case F5:  keystate(Shift) ? quick_load_snapshot(5) : quick_save_snapshot(5); break;
		case F6:  keystate(Shift) ? quick_load_snapshot(6) : quick_save_snapshot(6); break;
		case F7:  keystate(Shift) ? quick_load_snapshot(7) : quick_save_snapshot(7); break;
		case F8:  keystate(Shift) ? quick_load_snapshot(8) : quick_save_snapshot(8); break;

		case Home: // See also Numpad5!
			if (keystate(Ctrl)) {
		...
		case Numpad5: // See also Ctrl+Home!
			pan_view_reset();
			zoom_reset();
			break;

		case F12: toggle_huds();
