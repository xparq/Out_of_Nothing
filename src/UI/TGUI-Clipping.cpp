...
namespace tgui
{
        ...
        m_oldView{target.getView()}
    {
        ...
        //! Dunno how the original gets initialized in TGUI, but...:
        m_originalView = target.getDefaultView();
        ...
        {
            sf::View view({std::round(viewTopLeft.x), std::round(viewTopLeft.y)},
            ...
            view.setViewport(sf::FloatRect({topLeft.x, topLeft.y}, {size.x, size.y}));
            target.setView(view);
        }
        else // The clipping area lies outside the viewport
        {
            sf::View emptyView{{0, 0}, {0, 0}};
            emptyView.setViewport(sf::FloatRect({0, 0}, {0, 0}));
